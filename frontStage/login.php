...
<body>
    ...
    <div class="dowebok">
        ...
        <div class="form-item">
            <input id="username" name = "username" type="text" autocomplete="off" placeholder="Email">            
        </div>
        <div class="form-item">
...
$sql = "SELECT id FROM clubs WHERE username = '".$name."' AND password = '".$password."'";
$result = $conn->query($sql);
if ($result->num_rows > 0) {
    header("Location:http://www.e12300.com/clubWebsite/backStage/tables.php");
    while($row = $result->fetch_assoc()) {
        $clubID = $row["id"];
    } 
    setcookie("user", "$clubID", time()+1800,"/");
} else {
  echo $name;
  echo $password;
}

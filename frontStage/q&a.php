    ...
    <div class="heng_box heng_box_2 bg_gray">
        <div class="plan_com clearfix">
            ...
            <?php
                ...
                if($keyWord!=""){
                    $sql = "SELECT * FROM qna
                    WHERE CONCAT_WS(title,question,answer) LIKE '%".$keyWord."%';";
                }else{
                    $sql = "SELECT * FROM qna";
                }
                $conn->query($sql);
                ...
                while($row = $result->fetch_assoc()) {
                    $nameSql = "SELECT * FROM clubs WHERE id = '".$row["clubID"]."'";
                    $nameResult = $conn->query($nameSql);
                    while($nameRow = $nameResult->fetch_assoc()) {
                    $clubName = $nameRow["clubName"];
                    }
                    if($row["answer"]!=""){
                    echo "
                    <a href='qnaDetail.php?qnaID=".$row["id"]."'>
                    ...
                    <span class='rightSpan'>
                        <i style='margin-right:5px;' class='fa fa-clock-o fa-lg'></i>".$row["time"]."
                    </span>
                    <span class='leftSpan'>
                        <i style='margin-right:5px;' class='fa fa-user fa-lg' aria-hidden='true'></i>".$clubName."
                    </span>
                    </div>
                    </a>
                    ";
            ...
            <form action="q&a.php" name="form" method="POST" enctype="multipart/form-data">
            <div class="styledTextArea">
            <div class="top_all center PB15">
            <h4 class="MT0">—— Ask Your Question ——</h4>
            </div>
            <select name="clubs" class="clubs">
            <option style="color:gray" value="default" disabled="disabled" selected hidden>Select a club</option>
            <?php
                ...
                while($row = $result->fetch_assoc()) {
                    echo "
                    <option value='".$row["id"]."'>".$row["clubName"]."</option>
                        ";
                }      
            ?>
            </select>
                <div class="distancer">
                <input name="title" class="form-control" placeholder="Please enter the summary of your question"></input>
                </div>
                <div class="distancer">
                <textarea style="min-height:180px" name="content" class="form-control" rows="3" placeholder="Please input specefic description of the question here(optional)"></textarea>
                </div>
                <button name="submit" style="float:right; margin-top:70px" type="submit" method="post" class="btn btn-default" value="Submit">Submit</button>          
            </div>
            </div>
            </form>
        </div>
...
if(isset($_POST['submit'])){
    ...
    if($clubID!=""&&$title!=""){
        $sql2 = "INSERT INTO qna(clubID,question,title) VALUES ('$clubID', '$content','$title');";
        if ($conn->query($sql2) === TRUE) {
          echo "<script type = 'text/javascript'> 
                alert ('Your question is successfully sent to the club, answers will be posted on this page soon');
                </script>";
        } else {
          echo "<script type = 'text/javascript'>
          alert ('The question needs to be under 65535 letters.'); 
          </script>";
          echo $conn->error;
        }
    }else{
        echo "<script type = 'text/javascript'>
        alert ('Please enter a title or choose a club');
        </script>";
    }
}
?>